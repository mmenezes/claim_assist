...
public class BillsController {
	...
	public String saveBill() throws ParseException {
		SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd", Locale.ENGLISH);


		//Date date = formatter.parse(dateInString);
		for ( BillDataEnum billEnum:BillDataEnum.values()) {
			Bill bill = new Bill();
			bill.setClaim_id(billEnum.getClaimid());
			bill.setBill_id(billEnum.getBillid());
			System.out.println(billEnum.getBillDate());

			bill.setBill_date(new java.sql.Date((formatter.parse(billEnum.getBillDate())).getTime()));
			bill.setBill_description(billEnum.getBillDescr());
			bill.setBill_amount(billEnum.getAmt());
			claimService.saveBill(bill);
		}
		return "Bill Data added successfully.";
	}*/
